...
theorem add_left_cancel {a b c : R} (h : a + b = a + c) : b = c := by
  rw [← neg_add_cancel_left a c, ← h, add_comm, add_comm a b, add_neg_cancel_right]
...
lemma haha (k: G) : k * k = k -> k = 1 := by
  intro h
  have h2: k⁻¹ * k * k = k⁻¹ * 1 * k := by
    rw [mul_assoc, h, mul_assoc, one_mul]
  rw [mul_left_inv, one_mul, mul_assoc, one_mul, mul_left_inv] at h2
  exact h2
...
lemma mul_left_cancel (a b c: G) : a*b = a*c -> b = c := by
  intro h
